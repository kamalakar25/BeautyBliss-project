...
const Home = () => {
  ...
  // Function to handle card click and navigate to the service page
  ...
  const handleSubmitEnquiry = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/api/users/enquiries`, {
        ...
        email,
        // Add user info if needed, e.g., userId or userEmail
      });
        ...
        <div className="coverflow-container">
          <div className="coverflow">
            {[...slides, ...slides, ...slides].map((slide, index) => {
              ...
              const isActive = absOffset < 0.5; // Consider slide active if it's the closest to center
              const isVisible = absOffset <= 1; // Show details for slides within 1 position of center
              return (
                <div
